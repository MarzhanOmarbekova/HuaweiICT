...
export default function DashboardPage({ addToast }: PageProps) {
  ...
  if (!data)
    return (
      <EmptyState
        ...
        description="Make sure the Django server is running at http://127.0.0.1:8000"
      />
    );
  ...
  return (
    <div className="animate-in">
      <SectionHeader
        title="Dashboard"
        subtitle="Your energy portfolio at a glance"
      />
      ...
      >
        <StatCard
          ...
          value={`${(balance.available_credits || 0).toFixed(2)} kWh`}
          sub={`${(balance.locked_credits || 0).toFixed(2)} kWh locked`}
          color="accent"
          icon="FLASH"
        />
        <StatCard
          label="Coin Balance"
          value={(balance.coin_balance || 0).toFixed(2)}
          sub="VoltCoins"
          color="amber"
          icon="COIN"
        />
        <StatCard
          ...
          value={`${(stats.total_generated_kwh || 0).toFixed(1)} kWh`}
          sub="lifetime production"
          color="blue"
          icon="WIND"
        />
        <StatCard
          ...
          sub={`${stats.active_offers || 0} active offers`}
          color="violet"
          icon="DEVICE"
        />
      </div>
      ...
      >
        ...
        <Card>
          ...
          >
            <span>
              Available: {(balance.available_credits || 0).toFixed(2)}
            </span>
        ...
        <Card>
          ...
          {!recent_transactions?.length && (
            <EmptyState
              ...
              description="Start trading to see activity here"
            />
          )}
          {(recent_transactions || []).slice(0, 5).map((tx) => (
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                >
                  {tx.tx_type === "mint" ? "Energy Minted" : "P2P Trade"} -{" "}
                  {tx.energy_amount?.toFixed(2)} kWh
                </p>
                <p
                  style={{
                    fontFamily: "var(--font-mono)",
          ...
          >
            <div>
              ...
              <p style={{ fontSize: "13px", color: "var(--text-secondary)" }}>
                Add a device, record energy production, then list it on the
                marketplace.
              </p>
            </div>
