...
function getVariantStyle(v: string) {
  switch (v) {
    case 'primary': return { background: 'var(--accent)', color: 'var(--accent-text)' }
    case 'blue': return { background: 'var(--blue)', color: '#fff' }
    case 'danger': return { background: 'var(--red-dim)', color: 'var(--red)', border: '1px solid var(--red-dim)' }
    case 'ghost': return { background: 'transparent', color: 'var(--text-secondary)', border: '1px solid var(--border)' }
    default: return { background: 'var(--bg-card)', color: 'var(--text-primary)', border: '1px solid var(--border)' }
  }
}

function getSizeStyle(s: string) {
  switch (s) {
    case 'sm': return { padding: '6px 14px', fontSize: '12px' }
    case 'lg': return { padding: '13px 28px', fontSize: '15px' }
    default: return { padding: '9px 18px', fontSize: '14px' }
  }
}
...
export function Input({ label, error, hint, className, style, ...props }: InputProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
      {label && (
        <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          {label}
        </label>
      )}
      <input
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '22px', lineHeight: 1, padding: '2px 6px', borderRadius: '6px' }}
          >
            x
          </button>
        </div>
...
export function EmptyState({ icon, title, description }: { icon: string; title: string; description?: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--text-muted)' }}>
      ...
      <p style={{ fontSize: '15px', fontWeight: 500, color: 'var(--text-secondary)', marginBottom: '6px' }}>{title}</p>
...
export function SectionHeader({
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', gap: '16px' }}>
      <div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '22px', fontWeight: 700, letterSpacing: '-0.01em' }}>{title}</h1>
        {subtitle && <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '3px' }}>{subtitle}</p>}
      </div>
...
export function StatCard({
  ...
  return (
    <Card style={{ borderTop: `2px solid ${borderMap[color]}` }}>
      {icon && <div style={{ fontSize: '22px', marginBottom: '12px' }}>{icon}</div>}
      ...
      <p style={{ fontFamily: 'var(--font-mono)', fontSize: '26px', fontWeight: 600, color: colorMap[color] }}>{value}</p>
